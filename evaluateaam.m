...
%    fileresults=evaluateaam('aamlu(FHR)',data([data(:).trainingData]==1),'FHRMAdataset/traindata/')
%       ->Evaluate the Method Lu on the training dataset
%       The main criteria is then MADI=median([fileresults(:).MADI]);
%    fileresults=evaluateaam('FHRMAdataset/analyses/L_std.mat',data([data(:).trainingData]==1),'FHRMAdataset/traindata/')
...
for i=1:length(expertbase)
    ...
    [FHR,FHRraw,TOCO]=preprocess(FHR1,FHR2,TOCO,expertbase(i).unreliableSignal); %#ok<ASGLU>
    ...
    fileres=statscompare(FHR0,expertbase(i).baseline,baseline,ADExpert,ADMethod,expertbase(i).overshoots);
    fileres.filename=expertbase(i).filename;
    n=length(fieldnames(fileres));
    fileres=orderfields(fileres,[n 1:n-1]);
    fileresults(i)=fileres;%#ok<AGROW>
    fprintf('.');
end
fields=fieldnames(fileresults);
S.filename='Median';
for i=2:length(fields)
    M=median([fileresults.(fields{i})]);
    S.(fields{i})=M;
end
fileresults=[S fileresults];

f=uifigure;
uitable(f,'Data',struct2table(fileresults),'Unit','Normalized','Position',[0 0 1 1]);
end